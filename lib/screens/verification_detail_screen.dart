...
class _BlockchainInfo {
  ...
  factory _BlockchainInfo.fromJson(Map<String, dynamic> j) => _BlockchainInfo(
    ...
    network: j['network']?.toString() ?? 'BSC (BNB Smart Chain)',
  );
}
...
class _CertData {
  ...
  factory _CertData.fromJson(Map<String, dynamic> j, bool valid) {
    ...
    return _CertData(
      ...
      dimensions: j['dimensions']?.toString(),
      weight: j['weight']?.toString(),
      origin: j['origin']?.toString(),
      artisan: j['artisan']?.toString(),
      studio: j['studio']?.toString(),
      studioVerified: j['studioVerified'] == true,
      owner: j['owner']?.toString(),
      issuedAt: j['issuedAt']?.toString(),
      blockchain: b != null ? _BlockchainInfo.fromJson(b) : null,
      timeline: tl,
      isValid: valid,
    );
  }
  String get thumbnailUrl =>
      '${ApiConfig.baseUrl}/uploads/certificates/thumbs/$id.jpg';
...
class _VerificationDetailScreenState extends State<VerificationDetailScreen> {
  ...
  Future<void> _fetchCertificate() async {
    ...
    if (certId == null) {
      setState(() {
        _error = 'ID sertifikat tidak ditemukan';
        _loading = false;
      });
      return;
    }
    try {
      ...
      // API wraps response: { success, data: { certificate, isValid } }
      final data = (body['data'] ?? body) as Map<String, dynamic>;
      if (resp.statusCode == 200 && data['certificate'] != null) {
        setState(() {
          _cert = _CertData.fromJson(
  ...
  Future<void> _downloadPng(BuildContext context) async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text(
          'File PNG beresolusi tinggi ~4MB. Lanjutkan download?',
        ),
        actions: [
          TextButton(
            ...
            const SnackBar(
              content: Row(
                children: [
                  ...
                  SizedBox(width: 12),
                  Expanded(
                    child: Text('Tersimpan di Galeri → Album MajaCraft'),
                  ),
                ],
              ),
              backgroundColor: Color(0xFF653611),
        ...
        actions: [
          IconButton(
            ...
            onPressed: () {
              ...
              Share.share(
                'Sertifikat Phygital: ${widget.product.name}\nVerifikasi keaslian karya ini di:\n$url',
                subject: 'Sertifikat Phygital - ${widget.product.name}',
              );
            },
          ),
        ],
        ...
        children: [
          ...
          Padding(
            ...
            child: OutlinedButton.icon(
              onPressed: () => launchUrl(
                Uri.parse('https://majacraft.id/verifikasi/${cert.id}'),
          ...
          _card(
            ...
            Column(
              children: [
                ...
                _row(
                  'BERAT',
                  cert.weight != null ? '${cert.weight} gram' : '-',
                ),
                _row('ASAL DAERAH', cert.origin ?? '-'),
                _row('SENIMAN', cert.artisan ?? '-'),
                _row(
                  'STUDIO',
                  '${cert.studio ?? '-'}${cert.studioVerified ? '  ✓ Terverifikasi' : ''}',
                ),
                if (cert.owner != null) _row('PEMILIK', cert.owner!),
                _row(
                  'TANGGAL TERBIT',
                  cert.issuedAt != null ? _fmtDate(cert.issuedAt!) : '-',
                ),
            ...
            _card(
              ...
              Column(
                ...
                children: [
                  ...
                  if (cert.blockchain!.mintedAt != null)
                    _row('MINTING', _fmtDate(cert.blockchain!.mintedAt!)),
                  if (cert.blockchain!.txHash != null) ...[
                    _row(
                      'TX HASH',
                      '${cert.blockchain!.txHash!.substring(0, 14)}...',
                      mono: true,
                    ),
                      ...
                      children: [
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: () => launchUrl(
                              Uri.parse(
                                'https://bscscan.com/nft/0xcEa918A6f1472F913bD41a2E50aA59CE55553258/${cert.blockchain!.tokenId}',
                              ),
                              mode: LaunchMode.externalApplication,
                            ),
                        ...
                        Expanded(
                          child: OutlinedButton.icon(
                            onPressed: () => launchUrl(
                              Uri.parse(
                                'https://bscscan.com/tx/${cert.blockchain!.txHash}',
                              ),
                              mode: LaunchMode.externalApplication,
                            ),
          ...
          if (cert.timeline.isNotEmpty) ...[
            _card(
              ...
              'Riwayat Kepemilikan',
              Column(
                children: cert.timeline
                    .asMap()
                    .entries
                    .map(
                      (e) => _timelineItem(
                        e.value,
                        e.key == cert.timeline.length - 1,
                      ),
                    )
                    .toList(),
              ),
            ...
            Column(
              ...
              children: [
                ...
                _infoPoint(
                  Icons.visibility,
                  'Transparan & Terbuka',
                  'Siapapun dapat memverifikasi keaslian sertifikat ini kapan pun, tanpa perlu akun.',
                ),
                const SizedBox(height: 12),
                _infoPoint(
                  Icons.all_inclusive,
                  'Permanen Selamanya',
                  'Selama blockchain BSC beroperasi, catatan ini akan tetap ada tanpa batas waktu.',
                ),
                const SizedBox(height: 16),
                OutlinedButton.icon(
                  onPressed: () => launchUrl(
                    Uri.parse(
                      'https://bscscan.com/address/0xcEa918A6f1472F913bD41a2E50aA59CE55553258',
                    ),
                    mode: LaunchMode.externalApplication,
                  ),
                  icon: const Icon(Icons.open_in_new, size: 16),
                  label: const Text('Lihat Smart Contract di BSCScan'),
                  style: OutlinedButton.styleFrom(
                    foregroundColor: const Color(0xFF653611),
                    side: const BorderSide(color: Color(0xFF653611)),
                  ),
        ...
        Expanded(
          child: Text(
            ...
            style: TextStyle(
              ...
              color: highlight ? const Color(0xFF653611) : Colors.black87,
            ...
            child: Column(
              ...
              children: [
                ...
                if (ev.from != null || ev.to != null) ...[
                  ...
                  Text(
                    [
                      if (ev.from != null) 'Dari: ${ev.from}',
                      if (ev.to != null) 'Ke: ${ev.to}',
                    ].join(' → '),
                ...
                if (ev.txHash != null)
                  GestureDetector(
                    onTap: () => launchUrl(
                      Uri.parse('https://bscscan.com/tx/${ev.txHash}'),
                      mode: LaunchMode.externalApplication,
                    ),
                    child: Text(
                      'Lihat TX ↗',
  ...
  Widget _infoPoint(IconData icon, String title, String desc) => Row(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
